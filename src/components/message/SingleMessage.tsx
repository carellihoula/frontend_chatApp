...
const SingleMessage: React.FC<SingleMessageProps> = ({ message }) => {
  ...
  const formattedDate = message.timestamp.toLocaleDateString("fr-FR", {
    day: "2-digit",
    month: "2-digit",
    year: "numeric",
  });
    ...
    <SingleMessageStyled className="message flex items-start">
      <div className="avatar mr-4">
        <div className="w-16 rounded-full">
          <img src={message.senderPhoto} alt={message.content} />
        </div>
      </div>
      <div className="message-content">
        <p>
          <strong className="sender__name">{sender?.name}</strong>
